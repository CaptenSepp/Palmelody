...
def create_MIDI_note(base_note, int_gesture, oct):
...
def handle_note_change(current_gesture, previous_gesture, oct):
    if current_gesture != previous_gesture:
        # Turn off previous note if it exists
        if previous_gesture is not None:
            previous_note = create_MIDI_note(base_note, previous_gesture, 0)
            send_midi_note_off(previous_note)
            send_midi_note_off(previous_note + 12)

        # Turn on current note if it exists
        if current_gesture is not None:
            current_note = create_MIDI_note(base_note, current_gesture, oct)
    ...
    while cam.isOpened():
        ...
        if hands_detected.multi_hand_landmarks:
            ...
            for idx, hand_landmarks in enumerate(hands_detected.multi_hand_landmarks):
                ...
                if handedness == 'Right':
                    y_wrist_r = hand_landmarks.landmark[0].y
                ...
                if index_r is not None:
                    if vis_shoulder == True and vis_hip == True:
                        if y_wrist_r + (index_r * 0.01) < y_shoulder_avg:
                            oct=12
                            print(oct)
                        else:     
                            oct = 0
...
#---------------------------- turn gestures into midi ----------------------------------------------------------------#
        ...
        handle_note_change(current_gesture1,previous_gesture1, oct)
        ...
        if current_note1 is not None:
            ...
            send_midi_note_off(current_note1 + 12, channel=0)
