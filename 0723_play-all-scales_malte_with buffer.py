...
def set_base_note(base_note):
    ...
    if base_note in CHROMATIC_SCALE:
        return CHROMATIC_SCALE[base_note]
    else:
...
#-------------------------------------------------------- Hand recognition and CNN prediction -------------------------#
    ...
    while cam.isOpened():
        ...
        cv.putText(frame, 'base note: ' + base_note, (600, 400), cv.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2, cv.LINE_AA)
            ...
            for idx, hand_landmarks in enumerate(hands_detected.multi_hand_landmarks):
                ...
                if len(right_hand_lmks) == 42:
                    ...
                    with torch.no_grad():
                        ...
                        print(predicted_index_r.item())
                        index_r = predicted_index_r.item()
                        # print(index)
                        print (outputs)
                        print (predicted_value_r[0].item())
...
#---------------------------- turn gestures into midi ----------------------------------------------------------------#
        current_gesture1 = index_r
        current_gesture2 = index_l
        ...
        if current_note1 is not None:
            send_midi_note_off(current_note, channel=0)
            if current_note2 is not None:
                send_midi_chord_off(current_chord_notes, channel=1)
